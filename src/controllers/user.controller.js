...
const signupWithGoogle = async (req, res) => {
  try {
    // Extract data from request body
    const { name, email, password, profile_picture, phone_number } = req.body;
    ...
    if (existingUser) {
      ...
      return res.status(201).json({
        success: true,
        message: `${existingUser.dataValues.name} logged in successfully!`,
        accessToken,
        user:existingUser,
      });
    } else {
...
module.exports = { signupUser, loginUser, updateUser, signupWithGoogle };
